...
bool Datastructures::add_bite(BiteID id, const Name & name, Coord xy)
{
    ...
    sorted_bites_multimap.emplace(name,id);
    invalidate_cache();
...
/*
 * I think the original implementation is the most efficient possible with the
 * data structures you have. You got full optimization points, so the solution is
 * still effective.
 *
 * The optimized solution makes use of the multimap data structure, otherwise
 * using exactly the same logic to keep the bits in alphabetical order. This solution
 * does not need std::sort.
 */
std::vector<BiteID> Datastructures::get_bites_alphabetically() {
    if (!sorted_alphabetically) {
        ...
        // No need for sorting, multimap keeps bites in alphabetical order
        // Multimap <Name, ID>
        for (const auto& pair : sorted_bites_multimap) {
            // original had pair.first, no difference
            cached_bite_ids_alphabetical.push_back(pair.second);
        }
        /*
        This is good solution (without multimap data structure)

        std::sort(cached_bite_ids_alphabetical.begin(),
        cached_bite_ids_alphabetical.end(), [this](BiteID id1, BiteID id2) {
...
/*
 * This uses the same logic as get_bites_alphabetically() above. The solution is
 * generally good, but using a vector as a data structure to maintain order is not
 * perfect. This is because when you have to rebuild the data structure, you also
 * have to sort it. Sorting with the lambda function is clean and the logic is good.
 *
 * I would like to emphasize that the solution is good. Using multimap brings its
 * own challenges, as the number of data structures is increasing, which takes
 * more memory.
 */
