...
std::vector<BiteID> Datastructures::get_bites_alphabetically() {
    ...
    // "Lazy sorting", i.e. sorting only when it's necessary (e.g. when
    // bites have been added), gives function more efficiency. When sorting
    // is needed, it's more efficient to reserve space for the vector when
    // its' size is already known.
    // Copying id:s to return_vector is O(n) and sorting vectors'
    // Big(O)-notation is O(n log(n)) (at least I think so) so this functions
...
std::vector<BiteID> Datastructures::get_bites_distance_increasing() {
    ...
    // Code structure here is about the same as in alphabetically-function.
    // Therefore the same thoughts apply here aswell.
    // "Lazy sorting", i.e. sorting only when it's necessary (e.g. when
    // bites have been added), gives function more efficiency. When sorting
    // is needed, it's more efficient to reserve space for the vector when
    // its' size is already known.
